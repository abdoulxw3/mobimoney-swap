...
name: axelar-gmp
description: >
  Axelar General Message Passing (GMP) on Hedera. Use when sending or receiving
  cross-chain contract calls via Axelar Gateway (callContract,
  payNativeGasForContractCall), building AxelarExecutable receivers, wiring
  Hedera↔EVM destinations, or separating bridge transport from destination
  handlers (orchestrated / scheduled flows).
---
# Axelar GMP (Hedera)
